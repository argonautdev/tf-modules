...
inputs = {
  ...
  {{if .Spec.engine_version}}engine_version = "{{ .Spec.engine_version}}"{{else}}engine_version = "10.3.35"{{end}}
  {{if .Spec.db_instance_family}}family = "{{.Spec.db_instance_family}}"{{else}}family = "mariadb10.3"{{end}}
